...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function mountStripeCard() {
        ...
        if (!keyIsReal) {
            cardMount.innerHTML = '<p class="text-yellow-500 text-xs py-1">⚠️ Add your Stripe Publishable Key and Price ID in checkout.js to activate secure payments.</p>';
            return;
        }

        if (typeof Stripe === 'undefined') {
            cardMount.innerHTML = '<p class="text-red-400 text-xs py-1">Stripe failed to load. Check your internet connection.</p>';
            return;
        }

        stripe = Stripe(STRIPE_PUBLISHABLE_KEY);
        // Using Stripe Checkout Redirect, so we just display a message here instead of the Card Element
        cardMount.innerHTML = '<p class="text-gray-300 text-sm py-4 text-center">You will be redirected to the secure Stripe Checkout page to complete your purchase.</p>';
        cardMounted = true;
    }
    ...
    function loadAndRenderPayPal() {
        ...
        if (!keyIsReal) {
            container.innerHTML = '<p class="text-yellow-500 text-xs text-center py-4">⚠️ Add your PayPal Client ID in checkout.js to activate PayPal payments.</p>';
            return;
        }
        ...
        if (typeof paypal === 'undefined') {
            ...
            script.onerror = () => {
                container.innerHTML = '<p class="text-red-400 text-xs text-center py-4">PayPal failed to load. Check your Client ID.</p>';
            };
    ...
    function renderPayPalButton(container) {
        ...
        paypal.Buttons({
            ...
            onApprove: (data, actions) => actions.order.capture().then(() => showSuccessScreen(panel)),
            onError: err => {
