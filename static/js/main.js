document.addEventListener("DOMContentLoaded", () => {
    ...
    if (ageGate) {
        ...
        if (sessionStorage.getItem('ageVerified') === 'true' || localStorage.getItem('ageVerified') === 'true') {
            ...
            if (submitBtn) {
                ...
                submitBtn.addEventListener('click', () => {
                    ...
                    if (!dd || !mm || !yyyy || isNaN(dd) || isNaN(mm) || isNaN(yyyy) || dd > 31 || mm > 12 || yyyy < 1900) {
    ...
    const languages = [
        { code: 'en', name: 'English', flag: '<img src="https://flagcdn.com/20x15/ca.png" alt="EN" class="w-5 h-auto rounded-[2px] opacity-90 inline-block">' },
        { code: 'es', name: 'Spanish', flag: '<img src="https://flagcdn.com/20x15/es.png" alt="ES" class="w-5 h-auto rounded-[2px] opacity-90 inline-block">' },
        { code: 'fr', name: 'French', flag: '<img src="https://flagcdn.com/20x15/fr.png" alt="FR" class="w-5 h-auto rounded-[2px] opacity-90 inline-block">' },
        { code: 'de', name: 'German', flag: '<img src="https://flagcdn.com/20x15/de.png" alt="DE" class="w-5 h-auto rounded-[2px] opacity-90 inline-block">' }
    ];
    let selectedLang = localStorage.getItem('taprobaneLang') || 'en';

    // Create Toast Container (Global)
    const toastContainer = document.createElement('div');
    toastContainer.className = "fixed bottom-10 right-10 z-[100] flex flex-col gap-4 pointer-events-none";
    ...
    const navbars = document.querySelectorAll('#navbar .max-w-7xl > div.hidden, #mobile-menu');
    navbars.forEach(container => {
        const headerActions = document.createElement('div');
        headerActions.className = "flex items-center gap-6 pointer-events-auto h-full";
        ...
        if (container.id === 'mobile-menu') {
            headerActions.classList.add('px-8', 'py-10', 'border-t', 'border-white/5', 'justify-center', 'mt-auto', 'flex-col', 'gap-8');
            // Remove some absolute positioning for mobile dropdown
            langContainer.querySelector('div').classList.remove('absolute', 'right-0');
            langContainer.querySelector('div').classList.add('relative', 'w-full', 'mt-4', 'opacity-100', 'visible', 'bg-transparent', 'border-none', 'shadow-none');
    ...
    if (addToCartBtn) {
        addToCartBtn.addEventListener('click', () => {
            ...
            toast.className = "bg-dark-surface border border-gold border-l-4 shadow-[0_30px_60px_rgba(0,0,0,0.8)] text-white px-6 py-4 rounded-sm transform translate-x-[150%] transition-transform duration-500 flex items-center gap-4 pointer-events-auto";
